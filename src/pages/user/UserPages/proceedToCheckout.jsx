...
const CheckoutPage = () => {
  ...
  const fetchCartItems = async () => {
    try {
      setCartLoading(true)
      const response = await axiosInstance.get(`/user/getcartdetails/${user._id}`)
      ...
      setCartError(error.response?.data?.message || "Failed to fetch cart items")
    } finally {
  ...
  const validateForm = () => {
    ...
    if (!/^\d{10}$/.test(formData.mobile)) newErrors.mobile = "Mobile must be a 10-digit number"
  ...
  const handleAddressSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(`user/useraddress/${user._id}`, submitData)
            ...
            <CardContent>
              {error ? (
                <div className="text-red-500 py-4 text-center">
                  {error}
                  <button onClick={fetchAddresses} className="ml-2 text-blue-500 underline hover:text-blue-600">
                    Retry
                  </button>
                </div>
              ) : addresses.length === 0 ? (
                <div className="text-center py-4 text-gray-600">No addresses found. Please add a delivery address.</div>
              ) : (
                <div>
          ...
          <Card>
            ...
            <CardContent>
              {cartError ? (
                <div className="text-red-500 py-4 text-center">
                  {cartError}
                  <button onClick={fetchCartItems} className="ml-2 text-blue-500 underline hover:text-blue-600">
                    Retry
                  </button>
                </div>
              ) : cartItems.length === 0 ? (
                <div className="text-center py-4 text-gray-600">Your cart is empty</div>
              ) : (
                  ...
                  <div>
                    <div className="border-t pt-4">
                      ...
                      {validationError.payment && (
                        <Alert variant="warning" className="bg-yellow-50 border-yellow-200 mt-2">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-lg w-full mx-4">
            ...
            <form onSubmit={handleAddressSubmit} className="space-y-4">
              {errors.submit && (
                <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-2 rounded">{errors.submit}</div>
              )}
