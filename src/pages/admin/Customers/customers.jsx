...
import { User, Search, ChevronUp, ChevronDown, Filter, RefreshCw } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-black">
        <div className="text-lg animate-pulse text-blue-400">Loading customers...</div>
      </div>
    );
  }
          ...
          {['All', 'Active', 'Blocked'].map((filter) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm transition flex items-center ${
                activeFilter === filter ? 'bg-blue-600 hover:bg-blue-700' : 'bg-white/10 hover:bg-white/20'
              }`}
            >
            ...
            <Table>
              ...
              <TableBody>
                {filteredCustomers.map((customer) => (
                  <TableRow key={customer._id} className="hover:bg-white/5 transition-colors">
                    ...
                    <TableCell className="text-blue-200">{customer.email}</TableCell>
