...
import { Plus, Search, Edit, MoreVertical, Filter, Calendar, Tag, Eye, Percent } from "lucide-react"
...
const ViewOffersModal = ({ product, onClose,onUpdate  }) => {
  ...
  const handleRemoveOffer = async (offerId) => {
    try {
      const response = await axiosInstance.delete(`/admin/deleteProductOffers/${offerId}`)
      if (response.data.success) {
              ...
              <div key={index} className="bg-gray-700 p-4 rounded-lg">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-gray-300">{offer.offerValue}% off</p>
      ...
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setOfferData({ ...offerData, offerName: e.target.value })}
                className="w-full bg-gray-700 rounded-lg px-4 py-2"
                required
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Offer Value (%)</label>
              <input
                type="number"
                min="0"
                max="100"
                value={offerData.offerValue}
                onChange={(e) => setOfferData({ ...offerData, offerValue: e.target.value })}
                className="w-full bg-gray-700 rounded-lg px-4 py-2"
                required
              />
            </div>
          ...
          <div className="flex justify-end gap-4 mt-6">
            ...
            <button type="submit" className="px-4 py-2 bg-blue-600 rounded-lg hover:bg-blue-700 transition">
              Add Offer
            </button>
          </div>
        </form>
...
export function ProductPage() {
  ...
  const [selectedProductForViewing, setSelectedProductForViewing] = useState(null)
  ...
  const filterProducts = (productList, searchTerm, filter, date) => {
    return productList.filter((product) => {
      ...
      const matchesDate = !date || new Date(product.createdAt).toDateString() === new Date(date).toDateString()
  ...
  const handleListingToggle = async (productId, currentStatus) => {
    try {
      ...
      if (response.data.success) {
        setAllProducts((prev) =>
          prev.map((product) => (product._id === productId ? { ...product, status: newStatus } : product)),
        )
  ...
  const getStatusColor = (status) => {
    return status === "Published" ? "bg-green-500/20 text-green-500" : "bg-yellow-500/20 text-yellow-500"
  }
  ...
  const renderContent = () => {
    ...
    if (editTab && selectedProduct) {
      return <EditProduct product={selectedProduct} onSave={handleEditProduct} onCancel={() => setEditTab(false)} />
    }
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {products.map((product) => (
                  <tr key={product._id} className="border-b border-gray-700 hover:bg-white/5 transition">
                      ...
                      <div className="flex flex-col">
                        ...
                        {product.offer && (
                          <span className="text-sm text-gray-400 line-through">
                            ${(product.basePrice * (1 + product.offer.offerValue / 100)).toFixed(2)}
                          </span>
                        )}
                      </div>
                    </td>
